...
const FEATURE_DOCS = [
  ...
  },
  {
    ...
    topics: [
      { name: "Mailbox", content: "The Emails page (sidebar → Work → Emails) is a shared mailbox with Inbox, Unread, and Sent folders, full-text search, and a reading pane. Opening an inbound email marks it read; \"Mark unread\" puts it back in the triage queue. The Compose button writes from the mailbox directly — with or without linking a record — and Reply pre-fills the original sender, subject, and quoted body. Drafts autosave locally; Ctrl/⌘+Enter sends. If SMTP is not configured a clear warning banner appears and sending stays disabled." },
