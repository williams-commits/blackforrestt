...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const required: Permission = hasCoreEdit ? "ACCOUNTS_EDIT" : input.statusId !== undefined ? "ACCOUNTS_CHANGE_STATUS" : "ACCOUNTS_ASSIGN";
    const ctx = await scopedContext(required);
    return NextResponse.json({ data: await updateAccount(ctx, id, input) });
  } catch (error) {
