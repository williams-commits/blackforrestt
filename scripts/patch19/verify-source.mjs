...
check("registration creates unfunded accounts", /ensureUserLedgerAccount/.test(register) && /withSerializableRetry/.test(register) && /startingBalance > 0/.test(register), "new accounts are created retry-safe; demo funding is opt-in and only posts when DEMO_STARTING_BALANCE is positive");
...
check("market status banner shows quote source", /marketDataMode/.test(banner) && /Quote source|[Mm]arket data/.test(banner), "the trade terminal shows the quote source and connection state");
