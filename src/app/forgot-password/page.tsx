...
export default function ForgotPasswordPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!response?.ok) {
      ...
      setError(body?.error ?? "Password reset request failed.");
      return;
    }
    ...
    setMessage(
      body?.previewUrl
        ? "Development preview generated. Open the link below to reset the password."
  ...
  return (
    <main id="main-content" tabIndex={-1} className="min-h-screen bg-panel flex items-center justify-center p-4">
      <form onSubmit={submit} className="w-full max-w-sm bg-canvas border border-border rounded-lg p-6">
        ...
        <p className="text-xs text-text-muted mt-1 mb-5">Request a single-use 30-minute reset link.</p>
        <label className="text-xs text-text-muted" htmlFor="reset-email">Email</label>
        ...
        {previewUrl && <a href={previewUrl} className="block text-xs text-brand mb-3 hover:underline">Open development reset link</a>}
