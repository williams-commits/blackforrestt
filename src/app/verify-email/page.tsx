...
function VerifyEmail() {
  ...
  const [status, setStatus] = useState(token ? "Verifying…" : "Enter your email to resend verification.");
  ...
  async function resend(event: FormEvent) {
    ...
    setStatus(
      response?.ok
        ? body?.previewUrl
          ? "Development preview generated. Open the link below to verify this account."
          : "If eligible, a verification message has been sent."
  ...
  return (
    <section className="w-full max-w-sm bg-canvas border border-border rounded-lg p-6">
      ...
      {previewUrl && <a href={previewUrl} className="block text-xs text-brand mt-4 hover:underline">Open development verification link</a>}
      {!token && <form onSubmit={resend} className="mt-5">
        <label htmlFor="verification-email" className="text-xs text-text-muted">Email</label>
        ...
        <Button type="submit" loading={loading} className="w-full bg-brand text-white">Resend verification</Button>
      </form>}
      <Link href="/login" className="block text-xs text-brand text-center mt-4">Continue to sign in</Link>
    </section>
  );
}

export default function VerifyEmailPage() {
  return <main id="main-content" tabIndex={-1} className="min-h-screen bg-panel flex items-center justify-center p-4"><Suspense fallback={<p>Loading…</p>}><VerifyEmail /></Suspense></main>;
}
